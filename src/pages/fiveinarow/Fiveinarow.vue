...
// 初始化棋盘
function init() {
  checkerboard = Array.from({ length: 15 }, (_, y) =>
    Array.from({ length: 15 }, (_, x) => ({ x, y }))
  );
}
init();
// 下棋操作
...
function rightIncline(x: number, y: number, player: number) {
  ...
  for (let i = 1; i <= 5; i++) {
    ...
    if (
      tempX >= 0 &&
      tempY >= 0 &&
      checkerboard[tempY][tempX].player === player
    ) {
      len += 1;
    } else {
      break;
...
<template>
  <div class="five">
    <div class="container">
      <div class="header">
        <div class="status">当前玩家: {{ player === 1 ? "黑方" : "白方" }}</div>
        ...
        <tr v-for="(arr1, index) in checkerboard" :key="index">
          ...
          <td
            ...
            :class="{
              ...
              active: activeChess.x === pieces.x && activeChess.y === pieces.y,
            }"
            :key="pieces.x"
          ></td>
        </tr>
